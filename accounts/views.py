...
# importo la libreria requests
import requests
# Create your views here.


def register(request):
    #Capturar la data que esta enviando el usuario por el metodo POST
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # francei11740@gmail.com
            username = email.split("@")[0]
            user = Account.objects.create_user(first_name=first_name, last_name=last_name, email=email, username=username, password=password)
            user.phoner_number = phone_number
            user.save()


            # generar record en el user profile
            ...
            mail_subject = 'Por favor activa tu cuenta en Ecommerse'
        ...
        if user is not None:
            try:
                ...
                if is_cart_item_exist:
                    #Representa un array la coleccion de item que contiene el carrito de compras
                    cart_item = CartItem.objects.filter(cart=cart)

                    product_variation = []
                    for item in cart_item:
                        # obtengo la variaciones que se obtienes no estando en sesion
                        variation = item.variations.all()
                        product_variation.append(list(variation))

                        #obtenemos las variaciones cuando el user esta en sesion
            ...
            # http://127.0.01:8000/accounts/login/?next=/cart/checkout/
...
def dashboard(request):
    ...
    #logica para obtener userprofile
...
def forgotPassword(request):
    ...
    if request.method == 'POST':
        ...
        if Account.objects.filter(email=email).exists():
            ...
            # sintaxis para enviar el correo electronico de recuperacion
            ...
            messages.error(request, 'La cuenta de usuario no exite')
    ...
    if request.method == 'POST':
        ...
        if new_password == confirm_password:
            ...
            if success:
                ...
                messages.success(request, 'Su contraseña de actualizo correctamente!')
            ...
            messages.error(request, 'La contraseña no coincide con la confirmacion de password')
            return redirect('change_password')
